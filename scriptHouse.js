  ...
  const table_house = document.querySelector('.table_house')
...
const x = copiaHouse.forEach( member => member.state !== '' ? state.push(member.state) : null )

let stateNoRepetidos = new Set(state)

console.log(stateNoRepetidos)
...
stateNoRepetidos.forEach(state => {
  ...
  table_house.children[0].appendChild(encabezado)
  let tablebody = table_house.children[1]
  
  MembersHouse(copiaHouse,table_house);
  
  //Creacion de las filas de las tablas
  function MembersHouse(array,element){
    tablebody.innerHTML = ''
    console.log(array)
  ...
  formulario.addEventListener('change', (e)=>{
    ...
    arraylisten = copiaHouse.filter(member => member.state == checkedentradaslistadas[0] || checkedentradaslistadas[0] == 'ALL')  
    ...
    MembersHouse(arraylisten,table_house)
